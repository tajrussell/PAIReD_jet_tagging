...
def makeNtuplesPAIReDjointMC(inputFilePath, outputFilePath, batchsize = "10 MB",
    ...
    with uproot.open(inputFilePath) as inputFile:
        ...
        testOutputFilePath = outputFilePath.replace('PAIReD', 'PAIReD_test')
        with uproot.recreate(outputFilePath, compression=uproot.LZ4(4)) as outputFile, uproot.recreate(testOutputFilePath, compression=uproot.LZ4(4)) as testOutputFile:
            
            # create ROOT Tree in output file
            outputTree = outputFile.mktree("tree", outputTreeType, 
                                           title="PAIReD jets for training")
            testOutputTree = testOutputFile.mktree("tree", outputTreeType,
                                           title="PAIReD jets for training")
            ...
            for Events in inputTree.iterate(expressions=inputBranchNames,
                ...
                testOutputTree.extend(testDataPAIReD)
                #print(DataPAIReD['part'])
                N_processed += 1
