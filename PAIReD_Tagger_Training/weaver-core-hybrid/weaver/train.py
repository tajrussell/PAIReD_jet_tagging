...
parser.add_argument('--tensorboard-dir', type=str, default=None,
                    help='directory path for the tensorboard summary writer')
...
parser.add_argument('--loss-option', nargs=2, action='append', default=[],
                    help='options to pass to the loss function, e.g., `--loss-option "factor_reg" 0.1 --loss-option "factor_err" 0.2 --loss-option "baseline_err" 10.`')
...
def train_load(args):
    ...
    train_loader = DataLoader(train_data, batch_size=args.batch_size, drop_last=True, pin_memory=True,
                              ...
                              #changed by Jan from: persistent_workers=args.num_workers > 0 and args.steps_per_epoch is not None)
...
def model_setup(args, data_config):
    ...
    loss_options = {k: ast.literal_eval(v) for k, v in args.loss_option}
    ...
    # loss function
    try:
        loss_func = network_module.get_loss(data_config, **loss_options)
        _logger.info('Using loss function %s with options %s' % (loss_func, loss_options))
...
def save_root(args, output_path, data_config, scores, labels, observers):
    ...
    output = {}
    scores_cls, scores_reg, scores_err_plus, scores_err_minus = (scores, None, None, None) if args.train_mode == 'cls' else (None, scores, None, None) if args.train_mode == 'regression' else scores
    ...
    # write error estimate nodes
    if args.train_mode == 'hybrid':
        for idx in range(1, len(data_config.label_names)):
            name = data_config.label_names[idx]
            output['output_' + name + '_plus_sigma'] = scores_err_plus[:, idx-1]
            output['output_' + name + '_minus_sigma'] = scores_err_minus[:, idx-1]
...
def _main(args):
    ...
    if args.tensorboard:
        ...
        tb = TensorboardHelper(tb_dir=args.tensorboard_dir, tb_comment=args.tensorboard, tb_custom_fn=args.tensorboard_custom_fn)
    else:
        tb = None
