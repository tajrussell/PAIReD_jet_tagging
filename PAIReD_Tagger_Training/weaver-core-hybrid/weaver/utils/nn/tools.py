...
def train_classification(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
        ...
        for X, y, _ in tq:
            ...
            if tb_helper:
                tb_helper.write_scalars([
                    ...
                    ("LossCls/train", loss, tb_helper.batch_train_count + num_batches),
    ...
    if tb_helper:
        tb_helper.write_scalars([
            ("LossCls/train (epoch)", total_loss / num_batches, epoch),
...
def evaluate_classification(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    if tb_helper:
        ...
        tb_helper.write_scalars([
            ("LossCls/%s (epoch)" % tb_mode, total_loss / count, epoch),
...
def train_hybrid(model, loss_func, opt, scheduler, train_loader, dev, epoch, steps_per_epoch=None, grad_scaler=None, tb_helper=None):
    ...
    with tqdm.tqdm(train_loader) as tq:
        for X, y, _ in tq:
            ...
            with torch.cuda.amp.autocast(enabled=grad_scaler is not None):
                model_output = model(*inputs)
                logits = _flatten_preds(model_output[:, :-(3*n_reg)], label_mask)
                preds_reg = model_output[:, -(3*n_reg):-(2*n_reg)]
                preds_err_plus = model_output[:, -(2*n_reg):-(n_reg)]
            ...
            total_loss_err += loss_monitor['err']
            # if n_reg > 1:
            ...
            tq.set_postfix({
                'lr': '%.2e' % scheduler.get_last_lr()[0] if scheduler else opt.defaults['lr'],
                'LossCls': '%.5f' % loss_monitor['cls'],
                'LossReg': '%.5f' % loss_monitor['reg'],
                'LossErr': '%.5f' % loss_monitor['err'],
                'LossTot': '%.5f' % loss,
                # 'AvgLoss': '%.5f' % (total_loss / num_batches),
                'Acc': '%.5f' % (correct / num_examples),
                # 'AvgAcc': '%.5f' % (total_correct / count),
            ...
            # stop writing to tensorboard after 500 batches:  and num_batches < 500
            if tb_helper:
                tb_helper.write_scalars([
                    ("LossCls/train", loss_monitor['cls'], tb_helper.batch_train_count + num_batches), # to compare cls loss to previous loss
                    ("LossReg/train", loss_monitor['reg'], tb_helper.batch_train_count + num_batches),
                    ("LossErr/train", loss_monitor['err'], tb_helper.batch_train_count + num_batches),
    ...
    _logger.info('Train AvgLossCls: %.5f, AvgLossReg: %.5f, AvgLossErr: %.5f, AvgLossTot: %.5f, AvgAcc: %.5f, AvgMSE: %.5f, AvgMAE: %.5f' %
                 (total_loss_cls / num_batches, total_loss_reg / num_batches, total_loss_err / num_batches, total_loss / num_batches,
                 total_correct / count, sum_sqr_err / count, sum_abs_err / count))
    _logger.info('Train class distribution: \n    %s', str(sorted(label_counter.items())))

    if tb_helper:
        tb_helper.write_scalars([
            ("LossCls/train (epoch)", total_loss_cls / num_batches, epoch), # to compare cls loss to previous loss
            ("LossReg/train (epoch)", total_loss_reg / num_batches, epoch),
            ("LossErr/train (epoch)", total_loss_err / num_batches, epoch),
...
def evaluate_hybrid(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    with torch.no_grad():
        with tqdm.tqdm(test_loader) as tq:
            for X, y, Z in tq:
                ...
                logits = _flatten_preds(model_output[:, :-(3*n_reg)], label_mask).float()
                preds_reg = model_output[:, -(3*n_reg):-(2*n_reg)].float()
                preds_err_plus = model_output[:, -(2*n_reg):-(n_reg)].float()
                preds_err_minus = model_output[:, -(n_reg):].float()

                if not for_training:
                    scores_cls.append(torch.softmax(logits, dim=1).detach().cpu().numpy())
                    scores_reg.append(preds_reg.detach().cpu().numpy())
                    scores_err_plus.append(preds_err_plus.detach().cpu().numpy())
                    scores_err_minus.append(preds_err_minus.detach().cpu().numpy())
                    for k, v in y.items():
                        if k == '_label_':
                            labels[k].append(_flatten_label(v, label_mask).cpu().numpy())
                        else:
                ...
                if loss_func is not None:
                    ...
                    loss, loss_monitor = 0., {'cls': 0., 'reg': 0., 'err': 0.}
                ...
                total_loss_err += loss_monitor['err'] * num_examples
                e = preds_reg - label_reg
                abs_err = e.abs().sum().item()
                sum_abs_err += abs_err
                sqr_err = e.square().sum().item()
    ...
    if tb_helper:
        ...
        tb_helper.write_scalars([
            ...
            ("LossErr/%s (epoch)" % tb_mode, total_loss_err / count, epoch),
    ...
    if not for_training:
        ...
        scores_err_plus = np.concatenate(scores_err_plus)
        scores_err_minus = np.concatenate(scores_err_minus)
        labels = {k: _concat(v) for k, v in labels.items()}
    ...
    if for_training:
        ...
        # convert 2D labels/scores
        observers = {k: _concat(v) for k, v in observers.items()}
        return total_loss / count, (scores_cls, scores_reg, scores_err_plus, scores_err_minus), labels, observers


class TensorboardHelper(object):

    def __init__(self, tb_dir, tb_comment, tb_custom_fn):
        self.tb_comment = tb_comment
        from torch.utils.tensorboard import SummaryWriter
        if tb_dir is not None:
            current_time = time.strftime("%b%d_%H-%M-%S")
            tb_dir = os.path.join(
                tb_dir, "runs", current_time+self.tb_comment)
            _logger.info('Tensorboard summary writer path: %s' % tb_dir)
        self.writer = SummaryWriter(log_dir=tb_dir, comment=self.tb_comment)
